...
class ServicesCollector(BaseCollector):
            ...
            if aimesh_data:
                ...
                for node_mac, node_info in nodes.items():
                    if isinstance(node_info, dict) and "status" in node_info:
                        ...
                        AIMESH_NODE_STATUS.labels(
                            node_mac=node_mac, node_model=node_model
                        ).set(status_value)
    ...
    async def _collect_port_forwarding_metrics(self, metrics: Dict[str, Any]):
        ...
        try:
            port_forwarding_data = await self.router.async_get_data(
                AsusData.PORT_FORWARDING
            )
                ...
                for target, stats in ping_data.items():
                    if isinstance(stats, dict):
                        ...
                        if "response_time" in stats:
                            PING_RESPONSE_TIME.labels(target=target).set(
                                stats["response_time"]
                            )
                            metrics[f"ping_{target}_response_time"] = stats[
                                "response_time"
                            ]
