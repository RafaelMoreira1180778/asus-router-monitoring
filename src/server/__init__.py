...
class PrometheusServer:
    ...
    def __init__(
        self, config: ExporterConfig, collector_manager: MetricsCollectorManager
    ):
        self.config = config
    ...
    async def health_handler(self, request):
        ...
        connection_status = (
            "connected" if self.collector_manager.is_connected else "disconnected"
        )
...
"""
    ...
    async def collectors_handler(self, request):
        ...
        collectors_text += f"Total Data Types: {sum(len(types) for types in collector_info.values())}\n"
